...
contract MYieldFee is IContinuousIndexing, IMYieldFee, AccessControlUpgradeable, MYieldFeeStorageLayout, MExtension {
    ...
    function claimYieldFee() external returns (uint256) {
        ...
        address recipient_ = _getMYieldFeeStorageLocation().yieldFeeRecipient;
    ...
    function setClaimRecipient(
        ...
        _getMYieldFeeStorageLocation().claimRecipients[account] = claimRecipient;
    ...
    function earnerRate() public view virtual returns (uint32) {
        return
            UIntMath.safe32(
                (uint256(ONE_HUNDRED_PERCENT - yieldFeeRate()) * _currentEarnerRate()) / ONE_HUNDRED_PERCENT
            );
    }
    ...
    function yieldFeeRate() public view returns (uint16) {
        return _getMYieldFeeStorageLocation().yieldFeeRate;
    }

    /// @inheritdoc IMYieldFee
    function yieldFeeRecipient() public view returns (address) {
        return _getMYieldFeeStorageLocation().yieldFeeRecipient;
    }
    ...
    function _setYieldFeeRate(uint16 yieldFeeRate_) internal {
        if (yieldFeeRate_ > ONE_HUNDRED_PERCENT) revert YieldFeeRateTooHigh(yieldFeeRate_, ONE_HUNDRED_PERCENT);
